...
def debug(c):
    ...
    with c.cd("build"):
        c.run("JLinkGDBServerCLExe -select USB=801031598 -device RP2040_M0_1 -endian little -if SWD -speed 4000 \
                                -ir -noLocalhostOnly -nologtofile &",pty=True)
        c.run("arm-none-eabi-gdb {}".format(EXE),pty=True)
        
@task
def endDebug(c):
    """End Debug session with J-Link Edu Mini"""
    c.run('kill $(pgrep JLinkGDBServerCLExe)',pty=True)
...
def cmake(c):
    ...
    c.run("cmake .")
    # c.run("cmake -DPICO_BOARD=seeed_xiao_rp2040 .")
