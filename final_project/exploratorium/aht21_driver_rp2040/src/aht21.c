...
#else
AHT21_STATUS AHT21_init(AHT21 *s, uint16_t addr,i2c_inst_t *i2c)
{
	uint8_t count = 0;
	// busy_wait_ms(500); // 500ms power up
	s->addr = addr;
	s->hal_i2c = i2c;

	uint8_t param[2] = {0x30,0x00};

	i2c_write_blocking (s->hal_i2c,s->addr,(uint8_t*)AHT21_CMD_GET_STATUS,1,true);
	...
	while((s->status & AHT21_STATUS_INIT_RDY) != AHT21_STATUS_INIT_RDY && count == 3)
	{
		i2c_write_blocking (s->hal_i2c,s->addr,param,2,true);
		busy_wait_ms(10);
		i2c_read_blocking (s->hal_i2c,s->addr,&s->status, sizeof(s->status),false);
		busy_wait_ms(10);
		++count;
...
static AHT21_STATUS _AHT21_measure(AHT21 *s)
{
	...
	//Trigger measurement
	i2c_write_blocking (s->hal_i2c,s->addr,(uint8_t *) AHT21_CMD_MEASURE,1,true);
	...
	while((s->status & AHT21_STATUS_INIT_RDY) != AHT21_STATUS_INIT_RDY)
	{
		i2c_read_blocking (s->hal_i2c,s->addr,&s->status, sizeof(s->status),true);
		sleep_ms(10);
	}
		return AHT21_OKAY;
}
...
//	int result;
	...
	if(aht_res == AHT21_OKAY)
	{
		i2c_read_blocking (s->hal_i2c,s->addr,(uint8_t*)(s->raw_response), 6,true);
